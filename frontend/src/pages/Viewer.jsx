...
function SourceCoverage({ status }) {
  ...
  return <div style={{ textAlign: 'left', padding: '12px 14px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, margin: '0 0 18px' }}>
    ...
    <div style={{ fontSize: 13, color: '#334155', marginTop: 5, fontWeight: 750 }}>{status.jurisdiction?.resolved ? `${status.jurisdiction.state} → ${status.jurisdiction.city}` : 'City and state required'}</div>
    <div style={{ fontSize: 12, color: '#64748b', marginTop: 4 }}>{status.stateDocumentCount || 0} state records · {status.cityDocumentCount || 0} city records · {status.excludedJurisdictionDocumentCount || 0} unrelated records excluded</div>
    <div style={{ fontSize: 12, color: '#64748b', marginTop: 3 }}>{status.matchedDocumentCount} matched extracted documents · {status.matchedRequirementCount} requirements · {status.researchSourceCount || 0} filtered research links</div>
    ...
    <div style={{ fontSize: 10.5, color: '#64748b', marginTop: 9 }}>Only statewide and exact-city sources are available to AI. Catalogued links remain screening evidence until adoption, version, and project applicability are verified.</div>
  </div>
}
